...
std::string FileUtils::Convert(String str)
{
	const int32 srcLen = static_cast<int32>(str.size());
	...
	const int32 retLen = ::WideCharToMultiByte(CP_UTF8, 0, reinterpret_cast<wchar_t*>(&str[0]), srcLen, NULL, 0, NULL, NULL);
	ret.resize(retLen);
	::WideCharToMultiByte(CP_UTF8, 0, reinterpret_cast<wchar_t*>(&str[0]), srcLen, &ret[0], retLen, NULL, NULL);
